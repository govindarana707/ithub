...
$stmt = $conn->prepare("
    ...
    FROM discussions d 
    JOIN courses_new c ON d.course_id = c.id 
    WHERE c.instructor_id = ? AND d.parent_id IS NULL AND d.is_resolved = 0
");
if ($stmt === false) {
    $pendingCount = 0;
} else {
...
<div class="list-group sidebar-modern">
    ...
    <a href="create-course.php" class="list-group-item list-group-item-action <?php echo $current_page == 'create-course.php' ? 'active' : ''; ?>">
        <i class="fas fa-plus-circle me-2"></i> Create Course
    </a>
    <a href="students.php" class="list-group-item list-group-item-action <?php echo $current_page == 'students.php' ? 'active' : ''; ?>">
        <i class="fas fa-users me-2"></i> Students
        <span class="badge bg-info float-end" id="studentCount">0</span>
    </a>
    <a href="quizzes.php" class="list-group-item list-group-item-action <?php echo $current_page == 'quizzes.php' ? 'active' : ''; ?>">
        <i class="fas fa-brain me-2"></i> Quizzes
        <span class="badge bg-warning float-end" id="quizCount">0</span>
    </a>
    <a href="discussions.php" class="list-group-item list-group-item-action <?php echo $current_page == 'discussions.php' ? 'active' : ''; ?>">
        <i class="fas fa-comments me-2"></i> Discussions
        <?php if ($pendingCount > 0): ?>
            <span class="badge bg-danger float-end"><?php echo $pendingCount; ?></span>
        <?php endif; ?>
    </a>
    <a href="earnings.php" class="list-group-item list-group-item-action <?php echo $current_page == 'earnings.php' ? 'active' : ''; ?>">
